...
FLOW:{
	...
	WF_FLOWALIASISEMPTY	:	"请输入流程别名！",
	WF_FLOWTYPEISEMPTY	:	"请输入流程分类！",
	WF_BEGINISMORE		:	"不能创建多个开始节点。",
	WF_NOBEGIN		:	"流程必须有开始节点。",
	WF_NOENDOBJECT		:	"非自由流程必须有结束节点或者转入新流程节点。",
	...
	WF_SETTINGTRUE		:	"流程编辑数据设置正确。",
	WF_FORMNAMEISEMPTY	:	"请在流程属性中指定应用表单。",
	WF_EQFLOWMOREOBJECT	:	"等价流程不需要除文字标签以外的其他对象",
	WF_ENDOBJECTEXISTOUTLINE:	"流程的结束节点或者新流程节点存在指向其他节点的流向。",
	WF_ENDOBJECTMOREINLINE	:	"流程的结束节点或者新流程节点有且仅有一个来源流向。",
	WF_OBJECTOUTLINEISEMPTY	:	"非自由流程节点必须有指向其他节点的流向。",
	WF_LABELHTMLISEMPTY	:	"文字标签必须输入内容。",
	WF_TASKPROPERTYERROR	:	"环节属性存在问题：名称必须有且唯一，别名必须有且唯一，当办理人为现在指定时不能为空。",
	WF_TASKMOREOUTLINEFORCD	:	"当环节为条件分支起始点时不能有多个流向指向其他节点。",
	WF_SUBFLOWPROPERTYERROR	:	"新流程属性存在问题：名称必须有且唯一，别名必须有且唯一，且必须指定新流程。",
	WF_SUBFLOWMOREOUTLINEFORCD	:	"当新流程为条件分支起始点时不能有多个流向指向其他节点。",
	WF_CONDICTIONERROR	:	"判断点必须有名称，且只能有一个指向它的流向，而且这个流向必须从环节节点或者新流程结点出发。",
	WF_LINEPROPERTYERROR	:	"流向属性设置存在问题：必须有名称，当现在确定归档设置时归档数据库不能为空。",
	WF_LINESOURCEERRORFORCD	:	"条件分支流向来源节点必须是判断点。",
	WF_LINETOTASKERRORFORBEGIN	:	"开始节点不能立即指向结束节点、新流程节点或者判断点。",
