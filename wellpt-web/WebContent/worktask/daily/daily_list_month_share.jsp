						...
						$('#date01')
								.change(
										function() {
											...
											window.location.href = '${ctx}/worktask/daily/list_month_share.action?day='
													+ $('#date01').val();
										});
					});
</script>
<body>
	<div class="form-actions">
		...
		<a href="${ctx }/worktask/daily/list_share?userId=${userId}">按周查看</a>
			...
			<c:forEach items="${dailys}" var="item" varStatus="i">
				...
				<td style="height: 80px; margin: 0px; padding: 0px;">
					<div style="width: 100%; height: 100%; margin: 0px; padding: 0px;">
						<span style="display: block;<c:if test="${item.today=='true' }">color:red;</c:if>"> <c:if
								test="${item.no_month=='yes' }">
								<fmt:formatDate value="${item.date }" pattern="MM-dd" />
							</c:if> <c:if test="${empty item.no_month }">
								<fmt:formatDate value="${item.date }" pattern="dd" />
							</c:if><c:if test="${item.today=='true' }">今天</c:if>
						</span> &nbsp;&nbsp;
						<c:forEach items="${item.dailys }" var="subitem">
							<a href="#nogo" onclick="openwin('${subitem.uuid}')"> 有日志<br />
							</a>
						</c:forEach>
					</div>
