...
var tasks='';
<c:forEach var="item" items="${ps }" varStatus="i">
	 
 
  taskarray[${i.index}]={task_name:"${item.taskName}",task_type:"${item.taskType}",task_state:"${item.taskState}",target:"${item.target}",plan_begin_time:"${item.planBeginTime}",plan_end_time:"${item.planEndTime}",plan_work_count:"${item.planWorkCount}"};
</c:forEach> 
	...
	<script type="text/javascript"
		src="${ctx}/resources/worktask/js/report/workreport.js"></script>
</body>
</html>