	...
	<div id="container" class="schedule_group_list schedule_css">
			...
			<table width="100%">
				<tr>
					<td align="left">
						...
						<span class="s_taday" type="text" size="7" name="firstday">
							${firstday1}
						</span> 
						<a class="s_next_day" ldate="${firstday}" groupid="${groupid}" href="#"></a> 
						<span class="fromandto">(${from})<spring:message code="schedule.info.zhi" />
							(${to})
						</span>
					</td>
					<td align="left"></td>
					<td align="right">
						<a class="as_day" href="${ctx}/schedule/export_group.action?ldate=${firstday}&ctype=0&groupid=${groupid}">
							<spring:message code="schedule.btn.export" />
						</a> 
						<a class="asleader" ldate="${firstday}" groupid="${groupid}" href="#">
							<spring:message code="schedule.btn.shixiang" />
						</a>
					</td>
				</tr>
			</table>
		...
		<div class="content">
			<div class="content_group_list">
				<c:forEach items="${groupList}" var="rgoup">
					...
					<c:if test="${groupid==rgoup.uuid}">
						<div class="content_group_item activity">
							<a class="group_name" ldate="${ldate}" groupid="${rgoup.uuid}"
								href="#">${rgoup.groupName}</a> 
							<a class="editgroup" href="#"
								onclick="openLayerGroupSet2('${rgoup.uuid}','${rgoup.groupName}','${rgoup.userNames}','${rgoup.userIds}','${firstday}','${groupid}',1);"></a>
							<a class="delgroup" href="#"
								onclick="deleteGroup2('${rgoup.uuid}','${firstday}','${groupid}',1);"></a>
						</div>
					</c:if>
					<c:if test="${groupid!=rgoup.uuid}">
						<div class="content_group_item">
							<a class="group_name" ldate="${ldate}" groupid="${rgoup.uuid}"
								href="#">${rgoup.groupName}</a> <a class="editgroup" href="#"
								onclick="openLayerGroupSet2('${rgoup.uuid}','${rgoup.groupName}','${rgoup.userNames}','${rgoup.userIds}','${firstday}','${groupid}',1);"></a>
							<a class="delgroup" href="#"
								onclick="deleteGroup2('${rgoup.uuid}','${firstday}','${groupid}',1);"></a>
						</div>
				...
				<a class="addgroup"
					onclick="openLayerGroupSet('${firstday}','${groupid}',1);" href="#"><spring:message
						code="schedule.btn.add" />群组</a>
			</div>
			<div class="schedule_leader_list_content_div">
				<table class="schedule_leader_list_content" width="100%"
					height="100%" border="2">
					...
					<tr class="tr_title">
						...
						<c:forEach items="${weeks}" var="row">
							<c:if test="${row.sdate==now}">
								<td align="center" width="14%" class="tr_title_taday"><c:out
										value="${row.today}" /></td>
							</c:if>
							<c:if test="${row.sdate!=now}">
					...
					<c:forEach items="${leaders}" var="row2">
						...
						<tr class="tr_content" style="">
							<td class="tr_content_l1 tr_content_td">${row2.userName}</td>
									...
									<c:forEach items="${row2.oneList}" var="one">
										...
										<c:if test="${one.displayStartDate==now}">
											<font color="blue"><a href="#"
												onclick="openScheduleDialog('','','${mtype}','${ldate}','${userno}','2','5','${ctype}','${groupid}','${one.tipMethod}','${one.isComplete}','${week1.sdate}','${one.uuid}','${one.scheduleName}','${one.address}','${one.dstartDate}','${one.startTime}','${one.dendDate}','${one.endTime}','${one.isView}','${one.status}','${one.leaderNames}','${one.leaderIds}','${one.pleases}','${one.pleaseIds}','${one.views}','${one.viewIds}','${one.color}','${one.tip}','${one.tipDate}','${one.tipTime}','${one.repeat}','${one.startTime2}','${one.endTime2}','${one.tipTime2}','${one.creators}','${one.creatorIds}','${one.isLeaderView}','${one.inviteeNames}','${one.inviteeIds}','${one.acceptIds }','${one.acceptNames }','${one.refuseIds}','${one.refuseNames}','${one.tag.uuid}');">
													<font color="${one.color}"> <c:if
															test="${one.isComplete=='1'}">
															<strike><c:if
																	test="${one.startTime=='' or one.startTime==null}">
																	<spring:message code="schedule.info.allDay" />
																</c:if> <c:if
																	test="${one.startTime!='' and one.startTime!=null}">
																	<c:out value="${one.startTime}" />:<c:out
																		value="${one.startTime2}" />
																</c:if> <c:choose>
																	<c:when test="${fn:length(one.address)>3}">
																		<c:out value="${fn:substring(one.address,0,3)}.." />
																	</c:when>
																	<c:otherwise>
																		<c:out value="${one.address}" />
																	</c:otherwise>
																</c:choose> <c:choose>
																	<c:when test="${fn:length(one.scheduleName)>3}">
																		<c:out value="${fn:substring(one.scheduleName,0,3)}.." />
																	</c:when>
																	<c:otherwise>
																		<c:out value="${one.scheduleName}" />
																	</c:otherwise>
																</c:choose> </strike>
														</c:if> <c:if test="${one.isComplete!='1'}">
															<c:if test="${one.startTime=='' or one.startTime==null}">
																<spring:message code="schedule.info.allDay" />
															</c:if>
															<c:if test="${one.startTime!='' and one.startTime!=null}">
																<c:out value="${one.startTime}" />:<c:out
																	value="${one.startTime2}" />
															</c:if>
															<c:choose>
							...
							<td class="tr_content_td">
								<div
									style="width: 100px; overflow: hidden; display: inline; border: 1px; font-size: 12px;">
									<c:forEach items="${row2.twoList}" var="two">
										...
										<c:if test="${two.displayStartDate==now}">
											<font color="blue"><a href="#"
												onclick="openScheduleDialog('','','${mtype}','${ldate}','${userno}','2','5','${ctype}','${groupid}','${two.tipMethod}','${two.isComplete}','${week2.sdate}','${two.uuid}','${two.scheduleName}','${two.address}','${two.dstartDate}','${two.startTime}','${two.dendDate}','${two.endTime}','${two.isView}','${two.status}','${two.leaderNames}','${two.leaderIds}','${two.pleases}','${two.pleaseIds}','${two.views}','${two.viewIds}','${two.color}','${two.tip}','${two.tipDate}','${two.tipTime}','${two.repeat}','${two.startTime2}','${two.endTime2}','${two.tipTime2}','${two.creators}','${two.creatorIds}','${two.isLeaderView}','${two.inviteeNames}','${two.inviteeIds}','${two.acceptIds }','${two.acceptNames }','${two.refuseIds}','${two.refuseNames}','${two.tag.uuid}');">
													<font color="${two.color}"> <c:if
															test="${two.isComplete=='1'}">
															<strike><c:if
																	test="${two.startTime=='' or two.startTime==null}">
																	<spring:message code="schedule.info.allDay" />
																</c:if> <c:if
																	test="${two.startTime!='' and two.startTime!=null}">
																	<c:out value="${two.startTime}" />:<c:out
																		value="${two.startTime2}" />
																</c:if> <c:choose>
																	<c:when test="${fn:length(two.address)>3}">
																		<c:out value="${fn:substring(two.address,0,3)}.." />
																	</c:when>
																	<c:otherwise>
																		<c:out value="${two.address}" />
																	</c:otherwise>
																</c:choose> <c:choose>
																	<c:when test="${fn:length(two.scheduleName)>3}">
																		<c:out value="${fn:substring(two.scheduleName,0,3)}.." />
																	</c:when>
																	<c:otherwise>
																		<c:out value="${two.scheduleName}" />
																	</c:otherwise>
																</c:choose> </strike>
														</c:if> <c:if test="${two.isComplete!='1'}">
															<c:if test="${two.startTime=='' or two.startTime==null}">
																<spring:message code="schedule.info.allDay" />
															</c:if>
															<c:if test="${two.startTime!='' and two.startTime!=null}">
																<c:out value="${two.startTime}" />:<c:out
																	value="${two.startTime2}" />
															</c:if>
															<c:choose>
								...
								<div
									...
									<c:forEach items="${row2.threeList}" var="three">
										...
										<c:if test="${three.displayStartDate==now}">
											<font color="blue"><a href="#"
												onclick="openScheduleDialog('','','${mtype}','${ldate}','${userno}','2','5','${ctype}','${groupid}','${three.tipMethod}','${three.isComplete}','${week3.sdate}','${three.uuid}','${three.scheduleName}','${three.address}','${three.dstartDate}','${three.startTime}','${three.dendDate}','${three.endTime}','${three.isView}','${three.status}','${three.leaderNames}','${three.leaderIds}','${three.pleases}','${three.pleaseIds}','${three.views}','${three.viewIds}','${three.color}','${three.tip}','${three.tipDate}','${three.tipTime}','${three.repeat}','${three.startTime2}','${three.endTime2}','${three.tipTime2}','${three.creators}','${three.creatorIds}','${three.isLeaderView}','${three.inviteeNames}','${three.inviteeIds}','${three.acceptIds }','${three.acceptNames }','${three.refuseIds}','${three.refuseNames}','${three.tag.uuid}');">
													<font color="${three.color}"> <c:if
															test="${three.isComplete=='1'}">
															<strike><c:if
																	test="${three.startTime=='' or three.startTime==null}">
																	<spring:message code="schedule.info.allDay" />
																</c:if> <c:if
																	test="${three.startTime!='' and three.startTime!=null}">
																	<c:out value="${three.startTime}" />:<c:out
																		value="${three.startTime2}" />
																</c:if> <c:choose>
																	<c:when test="${fn:length(three.address)>3}">
																		<c:out value="${fn:substring(three.address,0,3)}.." />
																	</c:when>
																	<c:otherwise>
																		<c:out value="${three.address}" />
																	</c:otherwise>
																</c:choose> <c:choose>
																	<c:when test="${fn:length(three.scheduleName)>3}">
																		<c:out
																			value="${fn:substring(three.scheduleName,0,3)}.." />
																	</c:when>
																	<c:otherwise>
																		<c:out value="${three.scheduleName}" />
																	</c:otherwise>
																</c:choose> </strike>
														</c:if> <c:if test="${three.isComplete!='1'}">
															<c:if
																test="${three.startTime=='' or three.startTime==null}">
																<spring:message code="schedule.info.allDay" />
															</c:if>
										...
										<c:if test="${three.displayStartDate!=now}">
											<a href="#"
												...
												<font color="${three.color}"> <c:if
														...
														<c:choose>
															<c:when test="${fn:length(three.scheduleName)>3}">
																<c:out value="${fn:substring(three.scheduleName,0,3)}.." />
															</c:when>
															<c:otherwise>
																<c:out value="${three.scheduleName}" />
															</c:otherwise>
								...
								<div
									...
									<c:forEach items="${row2.fourList}" var="four">
										...
										<c:if test="${four.displayStartDate==now}">
											<font color="blue"><a href="#"
												onclick="openScheduleDialog('','','${mtype}','${ldate}','${userno}','2','5','${ctype}','${groupid}','${four.tipMethod}','${four.isComplete}','${week4.sdate}','${four.uuid}','${four.scheduleName}','${four.address}','${four.dstartDate}','${four.startTime}','${four.dendDate}','${four.endTime}','${four.isView}','${four.status}','${four.leaderNames}','${four.leaderIds}','${four.pleases}','${four.pleaseIds}','${four.views}','${four.viewIds}','${four.color}','${four.tip}','${four.tipDate}','${four.tipTime}','${four.repeat}','${four.startTime2}','${four.endTime2}','${four.tipTime2}','${four.creators}','${four.creatorIds}','${four.isLeaderView}','${four.inviteeNames}','${four.inviteeIds}','${four.acceptIds }','${four.acceptNames }','${four.refuseIds}','${four.refuseNames}','${four.tag.uuid}');">
													<font color="${four.color}"> <c:if
															test="${four.isComplete=='1'}">
															<strike><c:if
																	test="${four.startTime=='' or four.startTime==null}">
																	<spring:message code="schedule.info.allDay" />
																</c:if> <c:if
																	test="${four.startTime!='' and four.startTime!=null}">
																	<c:out value="${four.startTime}" />:<c:out
																		value="${four.startTime2}" />
																</c:if> <c:choose>
																	<c:when test="${fn:length(four.address)>3}">
																		<c:out value="${fn:substring(four.address,0,3)}.." />
																	</c:when>
																	<c:otherwise>
																		<c:out value="${four.address}" />
																	</c:otherwise>
																</c:choose> <c:choose>
																	<c:when test="${fn:length(four.scheduleName)>3}">
																		<c:out
																			value="${fn:substring(four.scheduleName,0,3)}.." />
																	</c:when>
																	<c:otherwise>
																		<c:out value="${four.scheduleName}" />
																	</c:otherwise>
																</c:choose> </strike>
														</c:if> <c:if test="${four.isComplete!='1'}">
															<c:if
																test="${four.startTime=='' or four.startTime==null}">
																<spring:message code="schedule.info.allDay" />
															</c:if>
															...
															<c:choose>
																<c:when test="${fn:length(four.scheduleName)>3}">
																	<c:out value="${fn:substring(four.scheduleName,0,3)}.." />
																</c:when>
																<c:otherwise>
																	<c:out value="${four.scheduleName}" />
																</c:otherwise>
										...
										<c:if test="${four.displayStartDate!=now}">
											<a href="#"
												...
												<font color="${four.color}"> <c:if
														...
														<c:if test="${four.startTime=='' or four.startTime==null}">
															<spring:message code="schedule.info.allDay" />
														</c:if>
														<c:if
															test="${four.startTime!='' and four.startTime!=null}">
							...
							<td class="tr_content_td">
								<div
									style="width: 140px; overflow: hidden; display: inline; border: 1px; font-size: 12px;">
									<c:forEach items="${row2.fiveList}" var="five">
										...
										<c:if test="${five.displayStartDate==now}">
											<font color="blue"><a href="#"
												onclick="openScheduleDialog('','','${mtype}','${ldate}','${userno}','2','5','${ctype}','${groupid}','${five.tipMethod}','${five.isComplete}','${week5.sdate}','${five.uuid}','${five.scheduleName}','${five.address}','${five.dstartDate}','${five.startTime}','${five.dendDate}','${five.endTime}','${five.isView}','${five.status}','${five.leaderNames}','${five.leaderIds}','${five.pleases}','${five.pleaseIds}','${five.views}','${five.viewIds}','${five.color}','${five.tip}','${five.tipDate}','${five.tipTime}','${five.repeat}','${five.startTime2}','${five.endTime2}','${five.tipTime2}','${five.creators}','${five.creatorIds}','${five.isLeaderView}','${five.inviteeNames}','${five.inviteeIds}','${five.acceptIds }','${five.acceptNames }','${five.refuseIds}','${five.refuseNames}','${five.tag.uuid}');">
													<font color="${five.color}"> <c:if
															test="${five.isComplete=='1'}">
															<strike><c:if
																	test="${five.startTime=='' or five.startTime==null}">
																	<spring:message code="schedule.info.allDay" />
																</c:if> <c:if
																	test="${five.startTime!='' and five.startTime!=null}">
																	<c:out value="${five.startTime}" />:<c:out
																		value="${five.startTime2}" />
																</c:if> <c:choose>
																	<c:when test="${fn:length(five.address)>3}">
																		<c:out value="${fn:substring(five.address,0,3)}.." />
																	</c:when>
																	<c:otherwise>
																		<c:out value="${five.address}" />
																	</c:otherwise>
																</c:choose> <c:choose>
																	<c:when test="${fn:length(five.scheduleName)>3}">
																		<c:out
																			value="${fn:substring(five.scheduleName,0,3)}.." />
																	</c:when>
																	<c:otherwise>
																		<c:out value="${five.scheduleName}" />
																	</c:otherwise>
																</c:choose> </strike>
														</c:if> <c:if test="${five.isComplete!='1'}">
															<c:if
																test="${five.startTime=='' or five.startTime==null}">
																<spring:message code="schedule.info.allDay" />
															</c:if>
															...
															<c:choose>
																<c:when test="${fn:length(five.scheduleName)>3}">
																	<c:out value="${fn:substring(five.scheduleName,0,3)}.." />
																</c:when>
																<c:otherwise>
																	<c:out value="${five.scheduleName}" />
																</c:otherwise>
										...
										<c:if test="${five.displayStartDate!=now}">
											<a href="#"
												...
												<font color="${five.color}"> <c:if
														...
														<c:if test="${five.startTime=='' or five.startTime==null}">
															<spring:message code="schedule.info.allDay" />
														</c:if>
														<c:if
															test="${five.startTime!='' and five.startTime!=null}">
								...
								<div
									...
									<c:forEach items="${row2.sixList}" var="six">
										...
										<c:if test="${six.displayStartDate==now}">
											<font color="blue"><a href="#"
												onclick="openScheduleDialog('','','${mtype}','${ldate}','${userno}','2','5','${ctype}','${groupid}','${six.tipMethod}','${six.isComplete}','${week1.sdate}','${six.uuid}','${six.scheduleName}','${six.address}','${six.dstartDate}','${six.startTime}','${six.dendDate}','${six.endTime}','${six.isView}','${six.status}','${six.leaderNames}','${six.leaderIds}','${six.pleases}','${six.pleaseIds}','${six.views}','${six.viewIds}','${six.color}','${six.tip}','${six.tipDate}','${six.tipTime}','${six.repeat}','${six.startTime2}','${six.endTime2}','${six.tipTime2}','${six.creators}','${six.creatorIds}','${six.isLeaderView}','${six.inviteeNames}','${six.inviteeIds}','${six.acceptIds }','${six.acceptNames }','${six.refuseIds}','${six.refuseNames}','${six.tag.uuid}');">
													<font color="${six.color}"> <c:if
															test="${six.isComplete=='1'}">
															<strike><c:if
																	test="${six.startTime=='' or six.startTime==null}">
																	<spring:message code="schedule.info.allDay" />
																</c:if> <c:if
																	test="${six.startTime!='' and six.startTime!=null}">
																	<c:out value="${six.startTime}" />:<c:out
																		value="${six.startTime2}" />
																</c:if> <c:choose>
																	<c:when test="${fn:length(six.address)>3}">
																		<c:out value="${fn:substring(six.address,0,3)}.." />
																	</c:when>
																	<c:otherwise>
																		<c:out value="${six.address}" />
																	</c:otherwise>
																</c:choose> <c:choose>
																	<c:when test="${fn:length(six.scheduleName)>3}">
																		<c:out value="${fn:substring(six.scheduleName,0,3)}.." />
																	</c:when>
																	<c:otherwise>
																		<c:out value="${six.scheduleName}" />
																	</c:otherwise>
																</c:choose> </strike>
														</c:if> <c:if test="${six.isComplete!='1'}">
															<c:if test="${six.startTime=='' or six.startTime==null}">
																<spring:message code="schedule.info.allDay" />
															</c:if>
															<c:if test="${six.startTime!='' and six.startTime!=null}">
																<c:out value="${six.startTime}" />:<c:out
																	value="${six.startTime2}" />
															</c:if>
															<c:choose>
								...
								<div
									...
									<c:forEach items="${row2.sevenList}" var="seven">
										...
										<c:if test="${seven.displayStartDate==now}">
											<font color="blue"> <a href="#"
												onclick="openScheduleDialog('','','${mtype}','${ldate}','${userno}','2','5','${ctype}','${groupid}','${seven.tipMethod}','${seven.isComplete}','${week1.sdate}','${seven.uuid}','${seven.scheduleName}','${seven.address}','${seven.dstartDate}','${seven.startTime}','${seven.dendDate}','${seven.endTime}','${seven.isView}','${seven.status}','${seven.leaderNames}','${seven.leaderIds}','${seven.pleases}','${seven.pleaseIds}','${seven.views}','${seven.viewIds}','${seven.color}','${seven.tip}','${seven.tipDate}','${seven.tipTime}','${seven.repeat}','${seven.startTime2}','${seven.endTime2}','${seven.tipTime2}','${seven.creators}','${seven.creatorIds}','${seven.isLeaderView}','${seven.inviteeNames}','${seven.inviteeIds}','${seven.acceptIds }','${seven.acceptNames }','${seven.refuseIds}','${seven.refuseNames}','${seven.tag.uuid}');">
													<font color="${seven.color}"> <c:if
															test="${seven.isComplete=='1'}">
															<strike><c:if
																	test="${seven.startTime=='' or seven.startTime==null}">
																	<spring:message code="schedule.info.allDay" />
																</c:if> <c:if
																	test="${seven.startTime!='' and seven.startTime!=null}">
																	<c:out value="${seven.startTime}" />:<c:out
																		value="${seven.startTime2}" />
																</c:if> <c:choose>
																	<c:when test="${fn:length(seven.address)>3}">
																		<c:out value="${fn:substring(seven.address,0,3)}.." />
																	</c:when>
																	<c:otherwise>
																		<c:out value="${seven.address}" />
																	</c:otherwise>
																</c:choose> <c:choose>
																	<c:when test="${fn:length(seven.scheduleName)>3}">
																		<c:out
																			value="${fn:substring(seven.scheduleName,0,3)}.." />
																	</c:when>
																	<c:otherwise>
																		<c:out value="${seven.scheduleName}" />
																	</c:otherwise>
																</c:choose> </strike>
														</c:if> <c:if test="${seven.isComplete!='1'}">
															<c:if
																test="${seven.startTime=='' or seven.startTime==null}">
																<spring:message code="schedule.info.allDay" />
															</c:if>
										...
										<c:if test="${seven.displayStartDate!=now}">
											<a href="#"
												...
												<font color="${seven.color}"> <c:if
														...
														<c:choose>
															<c:when test="${fn:length(seven.scheduleName)>3}">
																<c:out value="${fn:substring(seven.scheduleName,0,3)}.." />
															</c:when>
															<c:otherwise>
																<c:out value="${seven.scheduleName}" />
															</c:otherwise>
					...
					function() {
						...
						if (addCss == 'yes') {
							$.ajax({
								...
								url : "${ctx}/schedule/group_schedule.action",
								success : function(result) {
									$(".schedule_group_list").parent().html(
											result);
								}
							});
						} else {
							window.location.href = '${ctx}/schedule/group_schedule.action?ldate='
									+ ldate
									+ '&ctype=0&stype=1&groupid='
									+ groupid;
						}
					});
	//下一月
						...
						if (addCss == 'yes') {
							$
									.ajax({
										...
										success : function(result) {
											$(".schedule_group_list").parent()
													.html(result);
										}
									});
						} else {
							window.location.href = '${ctx}/schedule/schedule_setlist.action?ldate='
									+ ldate + '&ctype=1&groupid=' + groupid;
						}
					});
	//群组
