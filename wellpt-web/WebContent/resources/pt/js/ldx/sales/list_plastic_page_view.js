...
$(function() {
	...
	File.generateFile = function() {
		$.ajax({
			async : false,
			url : ctx + "/sales/plastic/page/preGenerate",
			success : function(data) {
				...
				json.buttons = {
					"生成编码" : function sure() {
						...
						JDS.call({
							service : "plasticPageService.generateFile",
							data : [ customer ],
							async : false,
							success : function(result) {
	...
	File.upgradeFile = function() {
		// window.open(ctx + "/transferData/fetchData?objectUuid=");
//		 window.open(ctx + "/transferData/fetchAttach?objectUuid=");
		var element = $(this);
		...
		if (uuids.length == 0) {
			...
			if (uuids.length > 1) {
				...
				JDS.call({
					service : "plasticPageService.upgradeFile",
					data : [ uuids.join(";") ],
					success : function(result) {
