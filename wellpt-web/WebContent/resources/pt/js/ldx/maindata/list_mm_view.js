...
$(function() {
	...
	$(".dataTr").live("dblclick",function(){
			...
			window.open(ctx + "/maindata/preAddMm?objectUuid=" + jsonObj.uuid);
		});	
	...
	File.editObject = function(parentUuid) {
		...
		if (uuids.length == 0) {
			...
			if (uuids.length > 1) {
				...
				window.open(ctx + "/maindata/preAddMm?objectUuid=" + uuids[0]);
			}
		}
	};

	File.addObject = function() {
		window.open(ctx + "/maindata/preAddMm?objectUuid=");
	};
	...
	File.deleteObjects = function() {
		...
		if (uuids.length == 0) {
			...
			oConfirm(fileManager.deleteConfirm, function() {
				JDS.call({
					service : "mainDataMmService.deleteObjects",
					data : [ uuids.join(";") ],
	...
	File.transferObjects = function() {
		...
		if (uuids.length == 0) {
			...
			JDS.call({
				service : "mainDataMmService.transferObjects",
				data : [ uuids.join(";") ],
				success : function(result) {
