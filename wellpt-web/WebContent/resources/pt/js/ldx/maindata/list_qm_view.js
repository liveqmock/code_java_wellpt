...
$(function() {
	...
	$(".dataTr").live("dblclick",function(){
			...
			window.open(ctx + "/maindata/preAddQm?objectUuid=" + jsonObj.uuid);
		});	
	...
	File.editObject = function(parentUuid) {
		...
		if (uuids.length == 0) {
			...
			if (uuids.length > 1) {
				...
				window.open(ctx + "/maindata/preAddQm?objectUuid=" + uuids[0]);
			}
		}
	};

	File.addObject = function() {
		window.open(ctx + "/maindata/preAddQm?objectUuid=");
	};
	...
	File.deleteObjects = function() {
		...
		if (uuids.length == 0) {
			...
			oConfirm(fileManager.deleteConfirm, function() {
				JDS.call({
					service : "mainDataQmService.deleteObjects",
					data : [ uuids.join(";") ],
	...
	File.transferObjects = function() {
		...
		if (uuids.length == 0) {
			...
			JDS.call({
				service : "mainDataQmService.transferObjects",
				data : [ uuids.join(";") ],
				success : function(result) {
