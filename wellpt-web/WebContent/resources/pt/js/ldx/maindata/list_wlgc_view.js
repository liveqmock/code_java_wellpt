...
$(function() {
	...
	$(".dataTr").live("dblclick",function(){
			...
			window.open(ctx + "/maindata/preAdd?wlgcUuid=" + jsonObj.uuid);
		});	
	...
	File.editObject = function(parentUuid) {
		...
		if (uuids.length == 0) {
			...
			if (uuids.length > 1) {
				...
				window.open(ctx + "/maindata/preAdd?wlgcUuid=" + uuids[0]);
			}
		}
	};

	File.addObject = function() {
		window.open(ctx + "/maindata/preAdd?wlgcUuid=");
	};
	...
	File.deleteObjects = function() {
		...
		if (uuids.length == 0) {
			...
			oConfirm(fileManager.deleteConfirm, function() {
				JDS.call({
					service : "mainDataWlGcService.deleteObjects",
					data : [ uuids.join(";") ],
	...
	File.transferObjects = function() {
		...
		if (uuids.length == 0) {
			...
			JDS.call({
				service : "mainDataWlGcService.transferObjects",
				data : [ uuids.join(";") ],
				success : function(result) {
					var errs = result.data.errStrs;
					var errMessage = "";
					for ( var i = 0; i < errs.length; i++) {
						errMessage += errs[i] + ",";
					}
					if ("" != errMessage) {
						alert(errMessage + "传递失败");
					}
