...
$(function() {
	...
	$(".dataTr").live("dblclick",function(){
			...
			window.open(ctx + "/maindata/preAddBaoGuan?objectUuid=" + jsonObj.uuid);
		});	
	...
	File.editObject = function(parentUuid) {
		...
		if (uuids.length == 0) {
			...
			if (uuids.length > 1) {
				...
				window.open(ctx + "/maindata/preAddBaoGuan?objectUuid="
						+ uuids[0]);
			}
		}
	};
	...
	File.deleteObjects = function() {
		...
		if (uuids.length == 0) {
			...
			oConfirm(fileManager.deleteConfirm, function() {
				JDS.call({
					service : "mainDataBaoGuanService.deleteObjects",
					data : [ uuids.join(";") ],
	...
	File.transferObjects = function() {
		...
		if (uuids.length == 0) {
			...
			JDS.call({
				service : "mainDataBaoGuanService.transferObjects",
				data : [ uuids.join(";") ],
				success : function(result) {
