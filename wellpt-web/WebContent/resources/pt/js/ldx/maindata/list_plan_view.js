...
$(function() {
	...
	$(".dataTr").live("dblclick",function(){
			...
			window.open(ctx + "/maindata/preAddPlanPrice?planUuid=" + jsonObj.uuid);
		});	
	...
	File.editObject = function(parentUuid) {
		...
		if (uuids.length == 0) {
			...
			if (uuids.length > 1) {
				...
				window.open(ctx + "/maindata/preAddPlanPrice?planUuid="
						+ uuids[0]);
			}
		}
	};
	...
	File.deleteObjects = function() {
		...
		if (uuids.length == 0) {
			...
			oConfirm(fileManager.deleteConfirm, function() {
				JDS.call({
					service : "mainDataPlanPriceService.deleteObjects",
					data : [ uuids.join(";") ],
	...
	File.transferObjects = function() {
		...
		if (uuids.length == 0) {
			...
			JDS.call({
				service : "mainDataPlanPriceService.transferObjects",
				data : [ uuids.join(";") ],
				success : function(result) {
