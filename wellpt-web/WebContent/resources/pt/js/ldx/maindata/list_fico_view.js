...
$(function() {
	...
	$(".dataTr").live("dblclick",function(){
			...
			window.open(ctx + "/maindata/preAddFico?objectUuid=" + jsonObj.uuid);
		});	
	...
	File.editObject = function(parentUuid) {
		...
		if (uuids.length == 0) {
			...
			if (uuids.length > 1) {
				...
				window
						.open(ctx + "/maindata/preAddFico?objectUuid="
								+ uuids[0]);
			}
		}
	};
	...
	File.addObject = function() {
		window.open(ctx + "/maindata/preAddFico?objectUuid=");
	};
	...
	File.deleteObjects = function() {
		...
		if (uuids.length == 0) {
			...
			oConfirm(fileManager.deleteConfirm, function() {
				JDS.call({
					service : "mainDataFicoService.deleteObjects",
					data : [ uuids.join(";") ],
	...
	File.transferObjects = function() {
		...
		if (uuids.length == 0) {
			...
			JDS.call({
				service : "mainDataFicoService.transferObjects",
				data : [ uuids.join(";") ],
				success : function(result) {
