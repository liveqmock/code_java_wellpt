...
function goToPage(num){
	...
	JDS.call({
		...
		data : [ pageInfo.searchType1,pageInfo.searchValue1,pageInfo.searchType2,pageInfo.searchValue2,pageInfo.searchType3,
		         ...
		         sgNum,pageInfo.currentPage,pageInfo.pageSize,pageInfo.sortField,'1'],
		async : false,
		success : function(result) {
			if(result.data.res=="success"){
		...
		buttons : {
			"确定" : function() {
				...
				if (fileType == ".xls" || fileType == ".xlsx") {
					$.ajaxFileUpload({
						url : ctx + "/productionPlan/bomPlan/uploadWg",// 链接到服务器的地址
						secureuri : false,
	...
	$("#mpsUploadDialog").dialog({
		...
		buttons : {
			"确定" : function() {
				...
				if (fileType == ".xls" || fileType == ".xlsx") {
					$.ajaxFileUpload({
						url : ctx + "/productionPlan/bomPlan/uploadMps",// 链接到服务器的地址
						secureuri : false,
...
function downloadWgTemplate(){
	...
	$('#downloadForm').attr("action",ctx + "/productionPlan/bomPlan/downloadWg?scrq="+scrq);
	$('#downloadForm').submit();
}
...
function downloadMesTemplate(){
	...
	$('#downloadForm').attr("action",ctx + "/productionPlan/bomPlan/downloadMes?scrq="+scrq);
	$('#downloadForm').submit();
}
...
function downloadMpsTemplate(){
	$('#downloadForm').attr("action",ctx + "/productionPlan/bomPlan/downloadMps");
	$('#downloadForm').submit();
}
...
function exportSuccessComplete(){
	$('#downloadForm').attr("action",ctx + "/productionPlan/bomPlan/exportSuccessComplete");
	$('#downloadForm').submit();
}
...
function exportReport(){
	...
	if(type!=""){
		...
		var action = ctx + "/productionPlan/bomPlan/exportExcelAll?reportType=" + type + "&searchType1="
