...
(function($) {
	...
	SubForm.prototype = {
		...
		init:function(){
			...
			jqGridColModels.push({name:'seqNO',index:'seqNO',width:'35px',fixed:true, align:"center", sortable:false});
			...
			for(var i in fields){
				...
				this.setModelWidth(model, field.width);
				//console.log(model.name + ":" + model.width);
				...
				if(typeof formula != "undefined" && $.trim(formula).length > 0){
					this.addFormula(field.name, formula);
				}
			...
			for(var i = 0; i < jqGridColModels.length; i ++){
			 ...
			 var btnUpId = "btn_up_" +  formUuid;
			 var btnDownId = "btn_down_" +  formUuid;
			 var btnAddSubId = "btn_add_sub_" + formUuid; 
			 var btnExpSubFormId = "btn_exp_subform_" + formUuid;
			 var btnImpSubFormId = "btn_imp_subform_" + formUuid;
			...
			// var options =  _paranentthis.cache.get.call(_paranentthis,cacheType.options);
			 ...
			 if(options.readOnly ){//只读
				 ...
				 if(subform.hideButtons == dySubFormHideButtons.show){//显示操作按钮，及其事件
					 ...
					 var btnAddElem = '<button  id="' + btnAddId +   '"   >' + dybtn.add  +'</button>' ;
					 var btnAddSubElem = "";
					 if(subform.supportSubRow){
						 btnAddSubElem = '<button  id="' + btnAddSubId +   '"   >' + dybtn.addSub  +'</button>' ;
					 }
					 var btnExpSubElem = "";
					 if(subform.showExpButton == "true" || subform.showExpButton == true){
						 btnExpSubElem = '<button  id="' + btnExpSubFormId +   '">' + dybtn.exp  +'</button>' ;//" onclick="javascript:window.excelImp4MainForm(\'dyform\')
					 }
					 var btnImpSubElem = "";
					 if(subform.showImpButton == "true" || subform.showImpButton == true){
						 btnImpSubElem = '<button  id="' + btnImpSubFormId +   '" >' + dybtn.imp  +'</button>' ;//" onclick="javascript:window.excelExp4MainFormEx(\'dyform\',\'' + subform.outerId + '\')
					 }
					
					 var btnEditElem =  '<button  id="' + btnEditId +   '"   >' + dybtn.edit  +'</button>' ;
					 var btnDelElem = '<button id="' + btnDelId +   '" >' + dybtn.del  +'</button>' ;
					 var btnUpElem = '<button id="' + btnUpId +   '" >上移</button>' ;
					 var btnDownElem = '<button id="' + btnDownId +   '" >下移</button>' ;
					 var strbtn1 = '<span id="'+subBtnId+'">';
					 var strbtn2 = '</span>';
					 var strSpace =  '&nbsp;&nbsp;&nbsp;&nbsp;';
					 if(subform.editMode == dySubFormEdittype.newWin){//在新窗口编辑
						  //而对于在窗口中编辑表单数据的需要“编辑“的按钮
						 jqGridCaption += strbtn1 + strSpace + btnAddElem + strSpace + btnAddSubElem + strSpace + btnEditElem  + strSpace +btnDelElem + strSpace + btnImpSubElem + strSpace + btnExpSubElem + strbtn2;
						 
						 _paranentthis.$("#" + btnAddId).live("click", function(event){//定义操作事件--添加行 
						 ...
						 _paranentthis.$("#" + btnImpSubFormId).live("click", function(event){
							 ...
							 try{
								 var formEl = this.form.id;
								 window.excelImp4MainForm(formEl);//abc
							 }catch(e){
								console.log(e);
							 }
							 return false;
						 ...
						 _paranentthis.$("#" + btnExpSubFormId).live("click", function(event){
							 ...
							 try{
								 var formEl = this.form.id;
								 window.excelExp4MainFormEx(formEl,subform.outerId);//abc
							 }catch(e){
								console.log(e);
							 }
							 return false;
						...
						//对于直接在jqgrid对表单数据进行编辑的方式，不需要“编辑“的按钮,直接在jqgrid中编辑即可
						 jqGridCaption += strbtn1 + strSpace + btnAddElem + strSpace + btnAddSubElem  + strSpace + btnDelElem + strSpace + btnUpElem + strSpace + btnDownElem + strSpace + btnImpSubElem + strSpace + btnExpSubElem + strbtn2; 
						 
						 _paranentthis.$("#" + btnAddId).live("click", function(evt){//定义操作事件--添加行 
					  ...
					  _paranentthis.$("#" + btnUpId).live("click", function(){//定义操作事件--上移
						 try{
							 ...
							 if(typeof selectedRowId == "undefined" || selectedRowId == null || selectedRowId.lengh == 0){  
								...
								if(!subform.supportSubRow){
									_this.upSubformRowDataEvent(formUuid, selectedRowId); 
								}
						 }catch(e){
								console.log(e);
						 }
					  ...
					  _paranentthis.$("#" + btnDownId).live("click", function(){//定义操作事件--下移
						 try{
							 ...
							 if(typeof selectedRowId == "undefined" || selectedRowId == null || selectedRowId.lengh == 0){  
								...
								if(!subform.supportSubRow){
									_this.downSubformRowDataEvent(formUuid, selectedRowId); 
								}
						 }catch(e){
								console.log(e);
						 }
			 ...
			 _tableelement.jqGrid({
				...
				colModel:jqGridColModels,
				 ...
				 treeGrid: true,
				  ...
				  shrinkToFit: this.getAutoWidth(),//横向流动条
				 // autowidth :false,
			    ...
			    jsonReader: {
			   	...
			   	afterInsertRow:function( rowId, rowdata, rowelem){
			   		...
			   		_this.invoke("afterInsertRow", rowdata );
			   	},
			   	onCellSelect:function(rowId, iCol,  cellcontent, e){
			   		...
			   		var control = $.ControlManager.getCtl(id);
			   	...
			   	//如果去掉下面的代码，第一次点击后，在光标移开之后没办法再还原为label状态
			   		...
			   		_this.invoke("onCellSelect", rowId, iCol,  cellcontent  );
			   	}, 
			   	onSelectRow: function(id){
			   		_this.selectedRowId = id;
			   		...
			   		_paranentthis.$("#" + btnUpId).attr("selectedRowId", id);
			   		_paranentthis.$("#" + btnDownId).attr("selectedRowId", id);
			   		_this.invoke("onSelectRow", id );
				}
