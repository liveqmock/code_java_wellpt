...
(function($) {
	...
	SubForm4Group.prototype = {
		...
		init:function(){
			...
			for(var i in fields){
				...
				if(typeof formula != "undefined" && $.trim(formula).length > 0){
					this.addFormula(field.name, formula);//将运算公式及公式依附的列放到缓存中
				}
			}
				 ...
				 grouping:true, 
		 		 groupingView : {
			   	...
			   	afterInsertRow:function( rowId, rowdata, rowelem){
			   		if(rowdata["controlable"]!=false){//在这里进行控件化,在插入后再进行分组的数据则这里不需要控件化
		 ...
		 * @param controlable 是否控件化
		 */
		//initializeControlFromRemoteFlag:false, 
		addRowData: function(formUuid, data, controlable){
			...
			data["controlable"] = controlable; //控件化由fillFormData接口处理
			...
			if( typeof controlable != "undefined" && controlable == false ){
		...
		fillFormData: function(formDatas){
			...
			for(var i =0; i < formDatas.length;  i++){
				...
				this.addRowData(this.options.formUuid, formData, false);
			}
			...
			var  colModel=$('#'+formUuid).jqGrid('getGridParam','colModel');
			 ...
			 //
			    ...
			    jQuery(obj).each(function(){
			 	...
			 	var groupvalue='';
			   	...
			   	for(var j = 0 ; j < colModel.length; j ++){
					...
					if($('#'+rowId).prev().is('tr[id*=ghead]')){//设置分组标题 
						//如果是分组收起的，获得分组图标后面的span，再设值。
						var groupspan=$('#'+rowId).prev().find('span');  
						 $(groupspan).after("<span>" +groupvalue + "</span>");
					}
			   ...
			   var time8 = (new Date()).getTime();
			    console.log("重新填充数据:" + (time8 - time7)/1000.0 + "s");
			...
			var i=0;
			   ...
			   var time4= (new Date()).getTime();
			    console.log("设置顺序:" + (time4 - time3)/1000.0 + "s");
