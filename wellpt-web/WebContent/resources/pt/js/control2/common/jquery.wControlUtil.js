...
(function($) {
	$.ControlUtil = {
			...
			setCommonCtrAttr:function(elment,options){
				 ...
				 elment.attr("isgetzhengwen",options.isgetzhengwen);//附件上传是否正文 TODO
*/			},
					 ...
					 if(!isaddrequire && fieldCheckRules.length > 0){
						 ...
						 for(var z = 0; z < rules.length; z ++ ){
							 if(rules[z].value == dyCheckRule.notNull ){//
								 rules2.push({value:rules[z].value, label:rules[z].label});
							 }
						 }
				 ...
				 var rulemap={};
					for(var k=0;k<rules.length;k++){
						if(dyCheckRule.notNull == rules[k].value){//非空
							...
							msgmap["isIdCardNo"]=rules[k].label;
					...
					if(columnProperty.length && $.trim(columnProperty.length).length > 0){//长度
		 ...
		 setSpanStyle:function (elment,textAlign,fontSize,fontColor,ctlWidth,ctlHight,val){
			 ...
			 elment.after("<span  id='span" + name + "'"+spanstyle+" >"+val+"</span>");
			},
			 ...
			 * @returns {String}
			 */
		  getStyleHtmlByParams:function(textAlign,fontSize,fontColor,ctlWidth,ctlHight,otherstyle){
				var cssTemp = "style = '";
				if(textAlign == "center") {
