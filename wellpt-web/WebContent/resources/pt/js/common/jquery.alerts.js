(function($) {
	$.alerts = {
		...
		alert: function(message ,callback ,title,width,height) {
			$.alerts.isSubmit = "no";
			callback_ = callback;
			if( title == null ) title = $.alerts.prompt;
			if( callback == null ) $.alerts.callback_ = callback;
			if( width == null ) width = $.alerts.width;
			if( height == null ) height = $.alerts.height;
		...
		alert3: function(message ,callback ,title,argument,width,height) {
			callback_ = callback;
			if( title == null ) title = $.alerts.prompt1;
			if( width == null ) width = $.alerts.width;
			if( height == null ) height = $.alerts.height;
			if( callback == null ) $.alerts.callback_ = callback;
			$.alerts.show(title, message, 'alert_',width,height);
			...
			$("body").append(
			  '<div id="popup_container">' +
			    ...
			    '<div id="popup_content">' +
			      ...
			      '<div id="close_button_div"><button id="submit_button">确定</button><button id="close_button">取消</button></div>'+
			      '<div id="close_time_div"><span class="close_time">'+$.alerts.time_s+'</span>秒后,窗口将自动关闭</div>'+
				'</div>' +
			  '</div>');
		...
		pageLock: function(status) {
			switch( status ) {
				case 'show':
					$.alerts._overlay('hide');
					$("BODY").append('<div id="page_lock"><img class="page_lock_icon" src="'+ctx+'/resources/pt/images/loading.gif"></div>');
					$("#page_lock").css({
						position: 'fixed',
						zIndex: 99998,
						top: '0px',
					...
					$(".page_lock_icon").css({
						marginTop:$(document).height()/2-50
					});
				break;
				case 'hide':
					$("#page_lock").remove();
