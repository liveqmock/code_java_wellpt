(function($) {
	...
	FileUpload.prototype = {
		...
		init : function() {
			...
			var html = '<div id="upload_div" class="upload_div">'
					+ '<span class="btn btn-success fileinput-button2">' + '<span class="file_icon"></span>'
					+ '<span class="add_icon">添加附件</span>' + '<input id="' + fileDivId
					+ '" name="files" type="file" class="fileupload_css" />' + '</span></div>'
					+ '<div class="files files_' + fileDivId + '">' + '<div class="fileList">'
					+ '<ul class="attach-list"></ul>' + '</div></div>';
			$element.append(html);
			options.formData.attach = this.nodeName;
			$("#" + fileDivId).fileupload(options).on('fileuploadadd', function(e, data) {
				pageLock("show");
				...
				$
						.each(
								...
								function(i, file) {
									...
									$files.push(file);
									var html = '<div><div class="files_div"><span class="file_icon"></span><p class="filename">'
											+ file.filename
											+ '</p>'
									...
									$("input[id=" + file.id + "]").click(
											$.proxy($this._onDeleteFile, $this, file.id));
		...
		_onDeleteFile : function(fileId) {
			...
			this.files = newFiles;
			$("input[id=" + fileId + "]").parent().parent().remove();
		}
	};
	$.fn.fileUpload = function(option) {
		...
		var element = this[0];
		var $this = $(element), data = $this.data("fileUpload"), options = $.extend({}, $this.data(),
