!function($) {
	// <!-- 签署意见 -->
	var str = '<div id="dlg_sign_opinion" class="form-horizontal" style="display:none;margin-bottom: 0px; padding-bottom: 0px;">'
			+ '<div class="nav-collapse nav-collapse-top collapse" style="padding: 0px; margin: 0px; border: none;">'
			+ '<ul class="nav pull-right" style="height:auto;background:none;border:none;color:#000;">'
			+ '<li class="dropdown user-avatar open"><a href="#" class="dropdown-toggle" data-toggle="dropdown" style="color:#000;"><span>常用意见</span></a>'
			+ '<ul id="common_opinion" class="dropdown-menu" style="left: -110px;">'
			+ '</ul></li>'
			+ '</ul>'
			+ '</div>'
			...
			+ '</div>'
			+ '</div>'
			+ '<div id="task_opinions" class="control-group" style="margin-bottom: 0px;">'
			+ '</div>' + '</div>';
	// <!-- 常用意见管理 -->
	var mgr = '<div id="dlg_opinion_mgr" style="display: none;">'
			...
			+ '<input id="opinion_category" type="text" placeholder="新分类" class="input-xlarge" style="width: 524px">'
			...
			+ '<div class="controls form_operate customButton" style="margin-left: 0px;margin-top: 0px;display:block;">'
			+ '<input id="opinion_content" type="text" placeholder="新意见" class="input-xlarge" style="width: 566px;">'
				...
				.click(
						function() {
							...
							var radio = $('<label class="radio inline label-opinion-category"><input type="radio" value="'
									...
									+ '" code="'
									+ id
									+ '" name="opinion_categories">' + category + '</label>');
	...
	function onGetAllOpinionCategoryBeansSuccess(result) {
		var data = result.data;
		var menu = "";
		if (data.length == 1) {
			$.each(data[0].opinions, function(i) {
				menu = menu + '<li class="opinion-menu-item"><a href="#"><span>' + this.content
						+ '</span></a></li>';
			});
		} else {
			$.each(data, function(i) {
				menu = menu + '<li class="dropdown-submenu pull-left"><a tabindex="-1" href="#">' + this.name
						+ '</a>';
		...
		menu = menu + '<li><a id="opinion_mgr" href="#"><span>管理</span></a></li>';
		$("#common_opinion").html(menu);
		$("#opinion_mgr").click($.proxy(onManagerOpinion, $("#opinion_mgr")));
		$(".opinion-menu-item").bind("click", function() {
			var $text = $("textarea[name=opinionText]", "#dlg_sign_opinion");
		...
		$(result.data).each(
				function(i) {
					...
					var radio = $('<label class="radio inline label-opinion-category">' + '<input id="'
							...
							+ '" name="opinion_categories" type="radio" value="' + this.name + '">'
							+ this.name + '</label>');
					$("#span_opinion_category").append(radio);
					// 选中第一个radio
					if (i == 0) {
	...
	var wfOpinion = '<div class="wf_opinion">'
			...
			+ '	<textarea name="sign-opinion-tx"></textarea>'
	...
	WorkFlowMiniOpinion.prototype = {
		...
		init : function() {
			...
			$(".sign-opinion-input textarea").keyup(function(e) {
				// 触发意见更改事件
