	...
	FileUpload4Image.prototype = {
		...
		init : function() {
			...
			if(this.getDataUuid()!=''&&this.getDataUuid()!=undefined){
				//初始化上传控件
				this.fileuploadobj.initWithLoadFilesFromFileSystem(setReadOnly,  $attachContainer,  this.options.enableSignature, mutiselect, this.getDataUuid() , id,ctlwidth,ctlhight);
			}else{
				this.fileuploadobj.init(setReadOnly,  $attachContainer,  this.options.enableSignature, mutiselect, [],ctlwidth,ctlhight);
			}
		}
	};
