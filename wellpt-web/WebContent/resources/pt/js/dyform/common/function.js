...
var loadFormDefinitions = function(formUuids){
	...
	$.ajax({
		...
		async : false,//同步完成
		type : "POST",
		data : {formUuids: JSON.cStringify(formUuids)} ,
		dataType : "json",
		success : function(data) { 
			definitionObjs = data; 
		}, 
...
var loadFormDefinitionsAndDefaultFormData = function(formUuids, formUuid){
	...
	$.ajax({
		...
		async : false,//同步完成
		type : "POST",
		data : {formUuids: JSON.cStringify(formUuids), formUuid: formUuid} ,
		dataType : "json",
		success : function(data) {
			datas = data; 
		}, 
...
var loadFormDefinitionDataByFormId = function(formId, dataUuid){
	...
	$.ajax({
		...
		async : false,//同步完成
		type : "POST",
		data : {formId: formId, dataUuid: dataUuid} ,
		dataType : "json",
		success : function(result) { 
			 if(result.success == "true" || result.success == true){ 
...
var toJSON = function()
{
	...
	*/
	  //alert(Object.create);
    if (typeof Object.create !== 'function') {//ie8下面没有该函数,所以在这里被定义
...
if(isIE8()){//对于ie8重写JSON.stringify
