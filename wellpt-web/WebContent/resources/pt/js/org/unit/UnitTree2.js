...
function PromptUnit(psTargetNames,paInitNames,paInitIDs,psTitle,pbMultiple,piSelectType,psNameType,pbShowType,psType,pbLoginStatus,psExcludeValues, callback){
	...
	*/
	...
	openDialog(ctx + "/resources/pt/js/org/unit/unit.htm", psTitle, 800, 500, "laArg", laArg, function(){
...
function UnitOnloadEvent(){
	...
	if (goUnitTree.initArg["ShowType"]!=true) ID_UnitType.style.display="none";
	...
	if(goUnitTree.initArg["Target"]!=null){
		...
		for(var j=0;j<laNode.length;j++){
			...
			liCount += laNode[j].childNodes.length;
		}
	...
	goUnitTree.showRole = $("#ID_ShowRole").attr("checked");
...
function bMakeButton(){
	...
	lsHtml +="</table>";
	ID_Button.innerHTML = lsHtml;
	ID_RMoveMenu.innerHTML = lsMenu;
	document.all.ID_RMoveToSelected.style.display="";
}
...
function bCreateTree(psType,pbExpandAll){
	...
	if(loNode==null){
		$.ajax({
				//url : ctx + "/unit/tree/unit/xml" + "?login="+(goUnitTree.initArg["LoginStatus"]?"1":"0")+"&all="+(pbExpandAll?"2":"1"),
				//type : "POST",
				url : "./MyUnit.xml",
				type : "GET",
				data : psType,
				...
				success: function(data){
					...
					if(loNode !=null){
						...
						//使用XML对象接口判断
						if (loNode.get(0).childNodes.length==1 && pbExpandAll!=true) {
		...
		var lsHTML = sGetNodeHTML(loNode,-1,1,pbExpandAll?true:false);
		goUnitTree.LTree.html(lsHTML);
		if (loNode.childNodes.length==1 && pbExpandAll!=true){ 
			goUnitTree.LTree.children(0).children(1).click();
		}
		if(goUnitTree.amended){
			goUnitTree.amended=false;
...
function sGetNodeHTML(poParent,piIndent,piTreeType,pbExpand){
	...
	for(var i=0;i<poParent.children().length;i++){
		var loNode = $(poParent.get(0).childNodes[i]);
		...
		var lbCheckbox = true;
		if (goUnitTree.initArg["SelectType"]==0 || lsType=="0"){
			lbCheckbox = false;
		...
		if ((piTreeType==1 || piTreeType==2) && goUnitTree.xmlObject["RTree"].get(0).childNodes.length>0){
			...
			if (laNode.length>0 && goUnitTree.initArg["Target"]==null) lbChecked=true;
		}
...
function oGetObjectByName(poObj,psName){
	var loDiv = poObj.parentElement;
	for(var i=0;i<loDiv.children.length;i++){
		if(loDiv.children[i].name==psName) return loDiv.children[i];
	}
	return null;
}
function toggle(poObj){
	...
	if (poObj.isOpen=="1"){
		poObj.isOpen="0";
		poObj.src=goUnitTree.expandImg;
		if(loCont!=null) loCont.style.display="none";
	}else{
		poObj.isOpen="1";
		poObj.src=goUnitTree.collapseImg;
		if(loCont!=null){
			loCont.style.display="";
			if (loCont.children.length==0){
				var loDiv = poObj.parentElement;
				$.ajax({
						//url : ctx + "unit_tree!unitXml.action?&all=0&login="+(goUnitTree.initArg["LoginStatus"]?"1":"0"),
						//type : "POST",
						url : "./MyUnit2.xml",
						type : "GET",
						data : loDiv.id,
						...
						success: function(data){
							var loNode = $(data).selectSingleNode("/units/unit[@id='"+loDiv.id+"']");
							...
							for(var i=0;i<loNode.get(0).childNodes.length;i++) loParent.appendChild($(loNode.get(0).childNodes[i]).cloneNode(true));
...
function MSurnameChangeEvent(psValue){
	if (psValue=="-0-"){
		...
		goUnitTree.MTree.html(sGetNodeHTML(goUnitTree.xmlObject["MTree"].firstChild,-1,2));
	}else{
...
function MTreeCurPage(pbNext){
	...
	goUnitTree.MTree.html(sGetNodeHTML(goUnitTree.xmlObject["MTree"].children(liIndex),-1,2));
}
function bSetMTree(psID){
	...
	var loNode = null;
	$.ajax({
		//url : ctx + "unit_tree!unitXml.action?type="+(goUnitTree.curType=="Dept"?"D":"U")+"&login="+(goUnitTree.initArg["LoginStatus"]?"1":"0"),
		//type : "POST",
		url : "./Leaf.xml",
		type : "GET",
		dataType : "xml",
...
function AddUnit(poObj,poParent){
	var loUnit = poParent.selectSingleNode("./unit[@id='"+poObj.value+"']");
	if(loUnit!=null) return;
	loUnit = $(goUnitTree.xmlDOM.createElement("unit"));
	loUnit.setAttribute("type",poObj.UnitType);
	loUnit.setAttribute("path",poObj.path);
	loUnit.setAttribute("isLeaf","1");
	var lsName = poObj.path;
	if(poObj.UnitType=="1"){
	...
	loUnit.setAttribute("id",poObj.value);
	var loSpan = oGetObjectByName(poObj,"-anchor");
	loUnit.setAttribute("title",loSpan.title.replace(/\n/g, ";"));
...
function selectCheckBox(poObj,defaultRNode){
	if(poObj.TreeType==3){
		var loUnit = goUnitTree.xmlObject["RTree"].selectSingleNode("//unit[@uid='"+poObj.parentElement.uid+"']");
		if(loUnit!=null) loUnit.setAttribute("checked",poObj.checked?"1":"");
	}else{
		if(poObj.NotSelect=="1"){
		...
		if(goUnitTree.initArg["Target"]==null){
			if(poObj.TreeType==1){
				...
				for (var i=0;i<laNode.length;i++){
					if(poObj.value==$(laNode[i]).val()){
						$(laNode[i]).attr("checked", poObj.checked);
						break;
					}
				}
			}
...
function dblSelectText(poObj){
	...
	if (poObj.parentElement.isLeaf=="1"){
		var loCheckBox = oGetObjectByName(poObj,"-checkbox");
		if(loCheckBox!=null) selectCheckBox(loCheckBox,goUnitTree.defaultRNode);
	}
}
function SelectAll(pbSelect,poTree){
	var lsSelected = ";";
	var laNode = poTree.getElementsByTagName("INPUT");
	for (var i=0;i<laNode.length;i++){
		if(laNode[i].NotSelect=="1") continue;
		laNode[i].checked=pbSelect;
		if(goUnitTree.initArg["Target"]!=null) continue;
		lsSelected += laNode[i].value + ";";
		if(pbSelect){
		...
		if(poTree.id="ID_LTree"){
			...
			for (var i=0;i<laNode.length;i++){
				if(lsSelected.indexOf(";"+$(laNode[i]).val()+";")!=-1){
					$(laNode[i]).attr("checked", pbSelect);
				}
			}
		}else{
			laNode = $("INPUT", goUnitTree.LTree);
...
function SelectedDelete(){
	...
	var lsDeleted = ";";
	for (var i=0;i<laNode.length;i++){
		if($(laNode[i]).attr("checked")!=true)continue;
...
function SelectedEmpty(){
	...
	for (var i=0;i<laNode.length;i++) laNode[i].parentNode.removeChild(laNode[i]);
	laNode = goUnitTree.xmlObject["RTree"].childNodes;
	for (var i=0;i<laNode.length;i++){
		var lsName = laNode[i].getAttribute("name");
		if (lsName!=null && lsName.lastIndexOf("(")!=-1) laNode[i].setAttribute("name",lsName.substring(0,lsName.lastIndexOf("("))+"(0)");
	}
	$("#ID_RMemberCount").text("0");
	goUnitTree.RTree.html(sGetNodeHTML(goUnitTree.xmlObject["RTree"],-1,3,true));
	if(goUnitTree.initArg["Target"]==null){
		laNode = $("INPUT", goUnitTree.LTree);
		for (var i=0;i<laNode.length;i++) laNode[i].checked=false;
		//laNode = goUnitTree.MTree.getElementsByTagName("INPUT");
		laNode = $("INPUT", goUnitTree.MTree);
		for (var i=0;i<laNode.length;i++) {
			$(laNode[i]).attr("checked", false);
...
function SelectedMove(pbUp){
	...
	if (pbUp){
		for (var i=0;i<laID.length;i++){
			...
			var loPrev = loNode.previousSibling;
			if (loPrev==null) break;
		...
		for (var i=laID.length-1;i>=0;i--){
			var loNode = goUnitTree.xmlObject["RTree"].selectSingleNode("//unit[@uid='"+laID[i]+"']");
			var loNext = loNode.nextSibling;
			if (loNext==null) break;
			var loParent = loNode.parentNode;
...
(function($) {
	$.unit = {
		openSelect : function(options) {
			...
			PromptUnit(options.psTargetNames, options.paInitNames,
					options.paInitIDs, options.psTitle, options.pbMultiple,
					options.piSelectType, options.psNameType,
					options.pbShowType, options.psType, options.pbLoginStatus,
					options.psExcludeValues, options.afterSelect);
		}
	};
})(jQuery);