(function() {
    ...
    function(editor) { 
        ...
        return {
					...
					{	id:"table_html",
                    	...
                    	onLoad:function(){
					        window.setTimeout(function(){
					        	var focusedDomPropertyUrl = editor.focusedDomPropertyUrl4Text; 
					        	 
					        	//以GET方式(不得以POST方式)加载属性窗口的html
					        	$("#" + containerID).load(focusedDomPropertyUrl, function(){
					        		textctl.initPropertyDialog(editor);//初始化属性窗口
					        	});
            ...
            onOk: function() {
            	var checkpass=$.ControlConfigUtil.ctlEditDialogOnOk(textctl,containerID);
            	if(!checkpass){
            		return false;
            	}
            },
            onCancel: function(){//退出窗口时清空属性窗口的缓存 
            	$.ControlConfigUtil.ctlEditDialogOnCancel(textctl,containerID);
            } ,
            onShow:function(){
              	var focusedDomPropertyUrl = editor.focusedDomPropertyUrl4Text; 
	        	//以GET方式(不得以POST方式)加载属性窗口的html
