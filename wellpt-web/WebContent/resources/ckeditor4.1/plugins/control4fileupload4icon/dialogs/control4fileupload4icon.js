(function() {
    ...
    function(editor) { 
        ...
        return {
                    	...
                    	onLoad:function(){
					        window.setTimeout(function(){
					        	...
					        	$("#" + containerID).load(focusedDomPropertyUrl, function(){
					        		fileupload4iconctl.initPropertyDialog(editor);//初始化属性窗口
					        	});
            ...
            onOk: function() {
            	var checkpass=$.ControlConfigUtil.ctlEditDialogOnOk(fileupload4iconctl,containerID);
            	if(!checkpass){
            		return false;
            	}
            },
            onCancel: function(){//退出窗口时清空属性窗口的缓存 
            	$.ControlConfigUtil.ctlEditDialogOnCancel(fileupload4iconctl,containerID);
            } ,
            onShow:function(){
              	var focusedDomPropertyUrl = editor.focusedDomPropertyUrl4FileUpload4Icon; 
	        	//以GET方式(不得以POST方式)加载属性窗口的html
	        	$("#" + containerID).load(focusedDomPropertyUrl, function(){
	        		fileupload4iconctl.initPropertyDialog(editor);//初始化属性窗口
	        	});
