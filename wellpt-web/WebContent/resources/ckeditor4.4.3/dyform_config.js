...
//	config.pasteFromWordRemoveFontStyles = false;//是否清楚word字体样式
	...
	config.contentsCss = ctx+'/resources/ckeditor4.4.3/dyformCkeditCss.css';	 
	config.tabIndex = 4;
...
CKEDITOR.getPluginContainerDomElement = function(plugin, focusedDomElement){
	...
	if(plugin == CkPlugin.SUBFORM){//从表插件 
		if(focusedDomElement.is( 'table' ) ||  focusedDomElement.hasAscendant( 'table' )){
			...
			||plugin == CkPlugin.CONTROL4TIMEEMPLOY
	...
	}else if(plugin == CkPlugin.BLOCK){//区块布局插件 
	...
	}else if(plugin == CkPlugin.TABS){//页签布局插件  
		if($(focusedDomElement).is( '.tabLayout' )  ){//页签布局
			pluginContainerDomElement = focusedDomElement;
		}
	}else if(plugin == CkPlugin.BUTTON){//按钮控件插件  
	...
	}else if(plugin == CkPlugin.MODELFIELD){//按钮控件插件 
		if(focusedDomElement.is( 'span' )  ){ 
			if(  $(focusedDomElement).attr("class") == "model_field"){//显示单据字段占位符
				pluginContainerDomElement = focusedDomElement;
			}
		}
	}else if(plugin == CkPlugin.MODELSUBFORM){//按钮控件插件 
		if(focusedDomElement.is( 'span' )  ){ 
			if(  $(focusedDomElement).attr("class") == "model_subform"){//显示单据字段占位符
				pluginContainerDomElement = focusedDomElement;
			}
		}
	}
...
function loadCommonJsFile(){
	...
	script.src = ctx + "/resources/ckeditor4.4.3/common.js";
...
function isDyFormControl(plugin,inputmode){
	...
	case  CkPlugin.TEXTAREACTL:
		...
		return checkpass;
		break;
	case  CkPlugin.CONTROL4TIMEEMPLOY:
		if(inputmode==dyFormInputMode.timeEmploy){
			checkpass=true;
		}
		return checkpass;
		break;
	case  CkPlugin.EMBEDDED:
