-- 政务外网存在的部门id升级
update xzsp_45.org_department t set t.id = 'D0000000441' where t.id = 'D0000000141';
update xzsp_45.org_department t set t.id = 'D0000000442' where t.id = 'D0000000142';
update xzsp_45.org_department t set t.id = 'D0000000443' where t.id = 'D0000000143';
update xzsp_45.org_department t set t.id = 'D0000000444' where t.id = 'D0000000144';
...
-- 用户相关信息合并
insert into xzsp_45.org_user
select * from xzsp_46.org_user t where t.uuid not in(
select uuid from xzsp_45.org_user
) and t.id not in(
select id from xzsp_45.org_user
);

insert into xzsp_45.org_department
select * from xzsp_46.org_department t where t.uuid not in(
select uuid from xzsp_45.org_department
) and t.id not in(
select id from xzsp_45.org_department
);

insert into  xzsp_45.org_department_user_job
select * from  xzsp_46.org_department_user_job t where t.user_uuid not in(
select j.user_uuid from xzsp_45.org_department_user_job j
) and exists (select t2.uuid from xzsp_45.org_department t2 where t2.uuid = t.department_uuid)
and exists (select t3.uuid from xzsp_45.org_user t3 where t3.uuid = t.user_uuid);

insert into  xzsp_45.org_user_role 
select * from xzsp_46.org_user_role t where t.user_uuid not in(
select j.user_uuid from xzsp_45.org_user_role j
) and exists (select t3.uuid from xzsp_45.org_user t3 where t3.uuid = t.user_uuid);
