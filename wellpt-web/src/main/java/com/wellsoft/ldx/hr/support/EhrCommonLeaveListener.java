...
@Service
@Transactional
public class EhrCommonLeaveListener implements TaskListener{
	...
	public void onCompleted(Event event) throws WorkFlowException {
		...
		// 通过从表id获得从表数据
		List<Map<String, Object>> childList = dyformdata.getFormDatasById("uf_scgl_gcsqd");
		Map<String,Object> child = childList.get(0);
		...
		String ts = (String) child.get("ts");//天数
		String zwdlr = (String) child.get("zwdlr");//职务代理人
		String dd = (String) child.get("dd");//出差地点
		String sy = (String) child.get("sy");//出差是由
		String uuid = (String) child.get("uuid");//流水单号
		BigDecimal xssB = new BigDecimal(xss);
		BigDecimal tsB = new BigDecimal(ts);
	    try {
...
//			@ii_rt varchar(100) output --返回值1/0/-1
			...
			//proc.setd
			proc.setInt(5,13);//--假别ID (注意：参数请传假别ID)
			proc.setBigDecimal(6, xssB);//--本次请假时数
			proc.setBigDecimal(7, tsB);//--本次请假天数
			proc.setString(8, sy);//--申请事由
