...
public class LeaveApplicationListener implements TaskListener{
	...
	public void onCompleted(Event event) throws WorkFlowException {
		...
		// 通过从表id获得从表数据
		List<Map<String, Object>> childList = dyformdata.getFormDatasById("userform_scgl_qjxx");
		String ygbh = (String)dyformdata.getFieldValue("gh");
		for(Map<String ,Object> child:childList){
			//String ygbh = (String) child.get("gh");//员工编号
			//String xm = (String) child.get("xm");//姓名
			//String bm = (String) child.get("bm");//部门
			//String gw = (String) child.get("gw");//岗位
			String nkssj = (String) child.get("StartTime");//开始时间
			String njssj = (String) child.get("EndTime");//结束时间
			...
			String ccsy = (String) child.get("Reason");//出差是由
			String uuid = (String) child.get("uuid");//流水单号
			String leaveType = (String)child.get("Type_QJ");//请假类别
			String relLeaveType = null;
			if(StringUtils.isNotBlank(leaveType)){
				relLeaveType = DyFormApiFacade.getRealValue(leaveType);
			}
			
			int leaveTypeInt = 0;
			if(StringUtils.isNotBlank(relLeaveType)){
				leaveTypeInt = Integer.parseInt(relLeaveType);//请假类别的真实值
			}
		    try {
...
//						@ii_rt varchar(100) output --返回值1/0/-1
				...
				//proc.setd
				proc.setInt(5, leaveTypeInt);//--假别ID (注意：参数请传假别ID)
