...
@Service
@Transactional
public class SrmServiceImpl extends BaseServiceImpl implements ISrmService {
	protected UniversalDao srmDao;

	public SrmServiceImpl() {
		srmDao = this.getDao("srmSessionFactory");
	}

	@Override
	public void execSql(String sql) {
		srmDao.getSession().createSQLQuery(sql).executeUpdate();
	}

	@SuppressWarnings("unchecked")
	@Override
	public List<Object> findListBySql(String sql) {
		return srmDao.getSession().createSQLQuery(sql).list();
	}
	...
	public List<QueryItem> queryForItemBySql(
		...
		SQLQuery localQuery = this.srmDao.getSession().createSQLQuery(
				sql.toString());
	...
	public SQLQuery createNativeQuery(String s, Map map) {
		SQLQuery query1 = this.srmDao.getSession().createSQLQuery(s);
		if (map != null)
	...
	public List<QueryItem> queryQueryItemData(String hql,
		...
		return this.srmDao.query(hql, values, QueryItem.class, pagingInfo);
	}
}
