...
@Service
@Transactional
public class ResumptionLeaveWirteSAPListener implements TaskListener{
	...
	public void onCompleted(Event event) throws WorkFlowException {
		...
		// 通过从表id获得从表数据
		List<Map<String, Object>> childList = dyformdata.getFormDatasById("userform_rsgl_sjqjxx");
		String begda = "";
		String endda = "";
		String beguz = "";
		String enduz = "";
		...
		for(Map<String,Object> child:childList){
			...
			String params = "{PI_PERNR:'" + pernr + "',PI_BEGDA:'" 
					+ begda + "',PI_ENDDA:'" + endda + "',PI_AWART:'" + child.get("Type_QJZ") + "'}";
			
			// 删除请假信息
			SAPRfcJson rfcjson = saprfcservice.RFC("sapConnectConfig", "ZHRI000802", params, 1, -1, null);
			// 解析返回结果
			if(rfcjson == null){
			...
			String params2 = "{PI_PERNR:'" + pernr + "',PI_BEGDA:'" 
					+ begda + "',PI_ENDDA:'" + endda + "',PI_BEGUZ:'" 
					+ beguz + "',PI_ENDUZ:'" + enduz + "',PI_AWART:'" + child.get("Type_QJZ") + "'}";
			// 请销假信息维护接口
			rfcjson = saprfcservice.RFC("sapConnectConfig", "zhri0008", params2, 1, -1, null);
			// 解析返回结果
			if(rfcjson == null){
				throw new WorkFlowException("第" + rowNum + "行：SAP接口没有返回消息！");
			}
