...
@Component
public class FicoViewDataSource extends AbstractDataSourceProvider {

	@Autowired
	private MainDataFicoService ficoService;
	...
	public List<QueryItem> query(Set<DataSourceColumn> viewColumns,
		...
		List<String> factorys = ficoService.findOwnFactory();
		if (null != factorys && factorys.size() > 0) {
			whereHql += " and factory in (:factory)";
			queryParams.put("factory", factorys);
		}
		List<QueryItem> items = ficoService.queryQueryItemData(
				"from Ficoview where " + whereHql + " " + orderBy, queryParams,
				pagingInfo);
		return items;
