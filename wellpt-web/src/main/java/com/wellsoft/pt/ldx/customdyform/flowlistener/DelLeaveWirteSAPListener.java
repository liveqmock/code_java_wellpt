...
@Service
@Transactional
public class DelLeaveWirteSAPListener implements TaskListener{
	...
	public void onCompleted(Event event) throws WorkFlowException {
		...
		String pernr = (String) (dataMap.get("gh")==null?"":dataMap.get("gh"));
		// 通过从表id获得从表数据
		...
		for(Map<String,Object> child:childList){
			...
			String params = "{PI_PERNR:'" + pernr + "',PI_BEGDA:'" 
					+ begda + "',PI_ENDDA:'" + endda + "',PI_AWART:'" 
					+ (child.get("Type_QJZ")==null?"":child.get("Type_QJZ")) + "'}";
			// 调用删除请假方法
