...
public class BoUserServiceImpl extends BaseServiceImpl implements BoUserService {
	...
	public Map saveBoview(User objectview) {
		...
		String boUser = "", boPwd = "";
		if (StringUtils.isNotEmpty(objectview.getBoUser())) {
			boUser = objectview.getBoUser();
		}
		if (StringUtils.isNotEmpty(objectview.getBoPwd())) {
			boPwd = objectview.getBoPwd();
		}
		String sql = "update org_user set BOUSER='" + boUser + "',BOPWD='" + boPwd + "' where uuid='"
				+ objectview.getUuid() + "'";
		this.execSql(sql);
		return hashmap;
	}
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<QueryItem> queryForItemBySql(Set<DataSourceColumn> viewColumns, String querySql,
			Map<String, Object> queryParams, String orderby, PagingInfo pagingInfo, Boolean isDistinct) {
		...
		SQLQuery localQuery = this.dao.getSession().createSQLQuery(sql.toString());
	...
	private long countSqlResult(StringBuffer s, Map<String, Object> queryParams, Boolean isDistinct) {
		String s1 = prepareCountSql(s.toString(), isDistinct);
		try {
			Long long1 = ((BigDecimal) createNativeQuery(s1, queryParams).uniqueResult()).longValue();
			return long1.longValue();
		} catch (Exception exception) {
			throw new RuntimeException((new StringBuilder()).append("hql can't be auto count, hql is:").append(s1)
					.toString(), exception);
		}
	}

	public SQLQuery createNativeQuery(String s, Map map) {
		SQLQuery query1 = this.dao.getSession().createSQLQuery(s);
		if (map != null)
	...
	private String prepareCountSql(String s, Boolean isDistinct) {
		String s1 = s, s2 = "";
		s1 = (new StringBuilder()).append("from ").append(StringUtils.substringAfter(s1, "from")).toString();
		...
		if (isDistinct) {
			...
			s2 = (new StringBuilder()).append("select count(distinct " + s6 + ") ").append(s1).toString();
		} else {
			s2 = (new StringBuilder()).append("select count(*) ").append(s1).toString();
		}
		return s2;
	}
}
