...
@Service
@Transactional
public class DqbsServiceImpl  extends BaseServiceImpl implements DqbsService{
	
	protected UniversalDao dqbDao;
	public DqbsServiceImpl() {
		dqbDao = this.getDao("mpsSessionFactory");
	}
	...
	public List<QueryItem> queryForItemBySql(Set<DataSourceColumn> viewColumns, String querySql,
		...
		SQLQuery localQuery = this.dqbDao.getSession().createSQLQuery(sql.toString());
	...
	public SQLQuery createNativeQuery(String s, Map map) {
		SQLQuery query1 = this.dqbDao.getSession().createSQLQuery(s);
		if (map != null)
