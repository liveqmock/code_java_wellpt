...
@Service
@Transactional
public class AssetsSaleListener implements TaskListener{
	...
	public void onCompleted(Event event) throws WorkFlowException {
		...
		if("计量器具".equals(dataMap.get("gdzclbz"))){
			...
			// 通过从表id获得从表数据
			List<Map<String, Object>> childList = dyformdata.getFormDatasById("uf_leedarson_gdzc_zczmxx");
			String labfile_no = "";
			...
			for(Map<String ,Object> child:childList){
				...
				if(lmsService.findLabFileById(labfile_no).size()>0){
					// SAP资产编号
					labfile_assetsno = child.get("jsfsapzcbm")==null?"":child.get("drfsapzcbm").toString();
					// 管理区域类别
					labfile_cls = child.get("jsfglqylbzsz")==null?"":child.get("drfglqylbzsz").toString();
					// 安装地点
					labfile_address = child.get("jsfazdd")==null?"":child.get("drfazdd").toString();
					// 使用人姓名
					labfile_claim = dataMap.get("jsfsyrxm").toString();
					// 主体归属
					labfile_mainhave = transformMainhave(dataMap.get("jsfgsdm").toString());
					// 成本中心代码
					labfile_costno = dataMap.get("jsfcbzxdm").toString();
					// 成本中心负责人
					labfile_usr = dataMap.get("jsfcbzxfzr").toString();
					// 接口人员
					labfile_interusr = dataMap.get("jsfjkry").toString();
