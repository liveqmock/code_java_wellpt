...
@Component
public class SdViewDataSource extends AbstractDataSourceProvider {

	@Autowired
	private MainDataSdService sdService;
	...
	public Set<DataSourceColumn> getAllDataSourceColumns() {
		...
		viewColumns.add(generateCol("factory", "factory", "工厂",
				ViewColumnType.STRING));
		viewColumns.add(generateCol("sdxszz", "sdxszz", "销售组织",
				ViewColumnType.STRING));
		return viewColumns;
	}
	...
	public List<QueryItem> query(Set<DataSourceColumn> viewColumns,
		...
		List<String> factorys = sdService.findOwnFactory();
		if (null != factorys && factorys.size() > 0) {
			whereHql += " and factory in (:factory)";
			queryParams.put("factory", factorys);
		}
		List<QueryItem> items = sdService.queryQueryItemData(
				"from Sdview where " + whereHql + " " + orderBy, queryParams,
				pagingInfo);
		return items;
