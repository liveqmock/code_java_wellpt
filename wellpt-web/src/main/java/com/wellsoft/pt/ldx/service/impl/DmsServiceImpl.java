...
@Service
@Transactional
public class DmsServiceImpl extends BaseServiceImpl implements IDmsService {
	protected UniversalDao dmsDao;

	public DmsServiceImpl() {
		dmsDao = this.getDao("dmsSessionFactory");
	}

	@Override
	public void execSql(String sql) {
		dmsDao.getSession().createSQLQuery(sql).executeUpdate();
	}

	@SuppressWarnings("unchecked")
	@Override
	public List<Object> findListBySql(String sql) {
		return dmsDao.getSession().createSQLQuery(sql).list();
	}
...
//		}
		...
		SQLQuery localQuery = this.dmsDao.getSession().createSQLQuery(
				sql.toString());
	...
	public SQLQuery createNativeQuery(String s, Map map) {
		SQLQuery query1 = this.dmsDao.getSession().createSQLQuery(s);
		if (map != null)
	...
	public List<QueryItem> queryQueryItemData(String hql,
		...
		return this.dmsDao.query(hql, values, QueryItem.class, pagingInfo);
	}
}
