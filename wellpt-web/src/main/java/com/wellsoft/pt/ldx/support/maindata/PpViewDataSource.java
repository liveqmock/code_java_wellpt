...
@Component
public class PpViewDataSource extends AbstractDataSourceProvider {
	...
	public List<QueryItem> query(Set<DataSourceColumn> viewColumns,
		...
		List<String> factorys = ppService.findOwnFactory();
		if (null != factorys && factorys.size() > 0) {
			whereHql += " and factory in (:factory)";
			queryParams.put("factory", factorys);
		}
		List<QueryItem> items = ppService.queryQueryItemData(
				"from Ppview where " + whereHql + " " + orderBy, queryParams,
				pagingInfo);
		return items;
