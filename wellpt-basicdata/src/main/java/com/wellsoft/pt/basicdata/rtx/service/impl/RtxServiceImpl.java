...
public class RtxServiceImpl implements RtxService {
	...
	public Boolean isRtxEnable() {
		...
		if (rtxDao.getAll().size() > 0) {
			result = rtxDao.getAll().get(0).getIsEnable();
		}
		return result;
	}
	...
	public void saveBean(RtxBean bean) {
		...
		if (StringUtils.isBlank(bean.getUuid())) {
			...
			rtx = this.rtxDao.get(bean.getUuid());
		}
		BeanUtils.copyProperties(bean, rtx);

		this.rtxDao.save(rtx);
	}
