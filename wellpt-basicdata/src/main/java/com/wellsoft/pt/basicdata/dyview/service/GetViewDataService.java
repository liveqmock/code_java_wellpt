 ...
 *
 */
public interface GetViewDataService {
	...
	//根据传入的模块id获取视图的列字段信息
	public abstract List<ViewColumn> getViewColumns(String id);

	//根据传入的模块id获取视图的列字段信息树
	public abstract List<TreeNode> getViewColumnsTree(String s, String id);

	//根据传入的viewUuid来获取视图的列属性信息
	public abstract List<ColumnDefinition> getFieldByView(String viewUuid);

	//获取自定义的按钮
	public abstract List<CustomButton> getCustomButtonByCode(String code);

	//获取有权限的自定义按钮
	public abstract void setCustomButtonRights(ViewDefinitionBean bean, Set<CustomButton> customButton);
